...
- Class materials (uploaded textbook/personal PDFs for this plan) are a separate source layer from trusted curriculum sources and from durable class memory. When a materials TOC is present: use list_class_materials / search_class_materials / read_class_material before inventing textbook facts; cite as `Material: material_id`. Prefer summary then section reads; do not dump full OCR markdown into plan_markdown.
