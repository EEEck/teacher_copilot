...
| Memory V3 (supersedes M2+M3) — capture discipline, ledger gate, single-call sweep, budgets | in progress — Phases 1–3 done and live (fixtures/goldens from real beta ledger; folding + section vocabulary + gate/decay; capture discipline wired into persistence). Phase 4 contract layer done: `validate_consolidation_ops` (structural-only, mem0 ID contract), `consolidation_failure_notice` (one plain notice, no zombie cards), `enumerate_memory_bullets`, `claims_from_clusters` — all tested. **Remaining Phase 4 core:** consolidation prompt + agent call (`OPENAI_SWEEP_MODEL`, high reasoning), swap `propose_memory_sweep_review` from two-pass packets to the single call, map ops→cards, migrate `test_memory_sweep_backend.py` + MBB trace, budgets at apply (Phase 5). Then M1b sweep brief (Phase 6). |
