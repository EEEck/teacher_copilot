...
- Golden assertions (initially `xfail`, flipped per phase):
  ...
  - gate: singleton inferred claims held; ≥2-session clusters eligible;
    explicit_ask always eligible; rejected-cluster suppressed;
...
2. Insert-time folding: exact normalized dup → reject (keep row as
   ...
   the same target → adopt the matched row's `cluster_key` (signal_count
   emerges from cluster size — no schema change).
3. History awareness on insert + listing: cluster matching an `applied` row →
   ...
   unless the new row is `explicit_ask`.
4. Promotion gate (pure function, unit-tested): eligibility per cluster —
   explicit_ask → always; inferred → distinct sessions ≥ 2, recency-weighted
   score with OpenClaw starting weights; below gate → stays `captured`.
...
1. Tighten `_DURABLE_PREFERENCE_MARKERS` to future-scoped phrases only;
   marker hit → `explicit_ask=true` (carried in `source`/new column-free
   encoding: `source="teacher_explicit"` stays the tag); no marker →
   inferred/low.
...
1. Build one consolidation input per sweep run: enumerate current bullets of
   ...
   call time); gate-passing claims with counts/dates; applied + rejected
   texts per target; today's date.
...
4. Map operations → existing `MemorySweepCandidate` card shape so
   ...
   bullet); DELETE → adjust-to-empty/compaction card; explicit_ask clusters
   → pinned queue "Explicitly requested".
