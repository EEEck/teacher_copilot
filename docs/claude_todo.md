...
| M1b — Memory Sweep brief (mem_v3 Phase 6) | implemented — `lib/sweep-brief.ts` + `memory-sweep-brief.tsx`; brief is the default surface (explicit asks pinned, New / Changed old→new / Already-covered sections, three uniform icon actions, sticky submit bar); detailed cards unchanged behind per-row "details" and a page toggle; StrictMode double-propose guard added (halves LLM cost per page open). tsc + vitest green; live UI pass pending. |
