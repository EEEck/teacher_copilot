...
| Memory V3 (supersedes M2+M3) — capture discipline, ledger gate, single-call sweep, budgets | **backend complete** — Phases 1–5 done and live: fixtures/goldens from real beta ledger; insert-time folding (cross-session re-statements reinforce clusters) + section vocabulary + gate/decay; capture discipline in the persistence path; `propose_memory_sweep_review` now runs gate → ONE consolidation call (`OPENAI_SWEEP_MODEL`) → structural validation → cards; two-pass packet machinery + lexical validators deleted (−2,182 lines); budget usage fed into the call (hermes-style page budgets already existed in `wiki/memory.py`). **Live-verified 2026-07-05**: demo-workspace ledger (the beta card-flood case) → 3 clean cards / 0 warnings; MBB/executive trace `passed=True` on `OPENAI_SWEEP_MODEL=gpt-5.4` (mini fails the add-vs-adjust judgment — control-tested; .env updated). Live runs also drove two fixes: no-change updates demoted to none deterministically, and same-section ≠ same-claim prompt rule. Open: DeepEval Tier-2/3 goldens. Next: M1b sweep brief (Phase 6, frontend PR). |
| M1b — Memory Sweep brief (mem_v3 Phase 6) | implemented — `lib/sweep-brief.ts` + `memory-sweep-brief.tsx`; brief is the default surface (explicit asks pinned, New / Changed old→new / Already-covered, plus separate **Student summary updates** last); three uniform icon actions, sticky submit bar; detailed cards unchanged behind per-row "details" and a page toggle; StrictMode double-propose guard added (halves LLM cost per page open). tsc + vitest green; live UI pass pending. |
