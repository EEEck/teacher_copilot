...
def chat_model_settings(
    ...
    ``minimal`` is rejected by the current gpt-5.4 / gpt-5.5 / gpt-5.6 Responses
    models (supported: none/low/medium/high/xhigh). Map it to ``low`` so utility
    ...
    if (
        ...
        and model
        and model.startswith(("gpt-5.4", "gpt-5.5", "gpt-5.6"))
    ):
