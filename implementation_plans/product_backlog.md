...
| **Staged-memory chat banner (all workflows)** | Discuss already shows a dismissible top-of-chat banner when `remember(...)` stages candidates (`StagedMemoryBanner`). Extend the same banner to Plan and Update Memory so all three workflows share one “staged for Memory Sweep” signal. Keep apply/reject on Sweep (or later in-chat cards); do not revive the post-save preference card. |
